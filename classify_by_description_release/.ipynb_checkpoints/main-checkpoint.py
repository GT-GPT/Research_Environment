...
gpt_descriptions, label_to_classname, n_classes = generate_gpt_descriptions(hparams)
...
label_encodings = compute_label_encodings(model,label_to_classname)


print("Evaluating...")
lang_accuracy_metric = torchmetrics.Accuracy('multiclass',num_classes=1000).to(device)
lang_accuracy_metric_top5 = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=5).to(device)

#clip_accuracy_metric = torchmetrics.Accuracy('multiclass',num_classes=1000).to(device)
#clip_accuracy_metric_top5 = torchmetrics.Accuracy('multiclass',num_classes=1000,top_k=5).to(device)
...
for batch_number, batch in enumerate(tqdm(dataloader)):
    ...
    for i, (k, v) in enumerate(description_encodings.items()): # You can also vectorize this; it wasn't much faster for me
        ...
        #image_description_similarity[i], _ = torch.topk(image_description_similarity[i], k=min(4,len(image_description_similarity[i][0])), dim=-1)
        image_description_similarity_cumulative[i] = aggregate_similarity(image_description_similarity[i])

    # create tensor of similarity means
    v = torch.stack(image_description_similarity_cumulative,dim=1)
    v_min = torch.min(v,dim=1,keepdim=True).values
    v_max = torch.max(v,dim=1,keepdim=True).values
    cumulative_tensor = (v - v_min) /(v_max - v_min)*(20) - 10
    prob = torch.nn.Softmax(dim=1)(cumulative_tensor)
    #print(prob[0], max(prob[0]))
    logp = torch.log(prob+ 1e-7)
    entropy = torch.sum(-prob*logp,dim=1)
    entropy = entropy.mean()
    
    entropys.append(entropy)
    if(batch_number % 1000 == 0):
        print(torch.mean(torch.stack(entropys)))
